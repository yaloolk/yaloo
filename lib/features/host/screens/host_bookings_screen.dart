...
class HostBookingsScreen extends StatefulWidget {
  ...
  @override State<HostBookingsScreen> createState() => _HostBookingsScreenState();
}
...
class _HostBookingsScreenState extends State<HostBookingsScreen>
  ...
  Future<void> _respond(String bookingId, String action) async {
    ...
    if (action == 'reject') {
      ...
      note = await showDialog<String>(context: context, builder: (_) => AlertDialog(
    ...
    final ok = await prov.respondToBooking(bookingId: bookingId, action: action, hostResponseNote: note);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(ok ? 'Booking ${action == "accept" ? "accepted ✓" : "declined"}' : prov.hostError),
  ...
  Widget build(BuildContext context) {
    return Consumer<StayBookingProvider>(builder: (_, prov, __) {
        ...
        body: Column(children: [
          ...
          Container(
            ...
            child: SafeArea(bottom: false, child: Column(
              ...
              children: [
                Padding(padding: EdgeInsets.fromLTRB(20.w, 16.h, 20.w, 0), child: Row(children: [
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Bookings', style: TextStyle(color: Colors.white, fontSize: 24.sp,
                        fontWeight: FontWeight.w800, letterSpacing: -0.5)),
                    Text('Manage your guest requests',
                        style: TextStyle(color: Colors.white.withOpacity(0.72), fontSize: 13.sp)),
                  ]),
                  const Spacer(),
                  if (requests.isNotEmpty)
                    Container(padding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 6.h),
                        ...
                        child: Text('${requests.length} pending',
                ...
                TabBar(
                  ...
                  indicator: UnderlineTabIndicator(borderSide: const BorderSide(width: 2.5, color: Colors.white),
                      insets: EdgeInsets.symmetric(horizontal: 20.w)),
                  tabs: [
                    _tabLabel('Requests', requests.length, _amber),
                    _tabLabel('Upcoming', confirmed.length, _green),
                    _tabLabel('History', history.length, _gray),
                  ],
                ),
              ],
            )),
          ...
          // ── Body ───────────────────────────────────────────────────
          Expanded(child: prov.hostLoading
              ? const Center(child: CircularProgressIndicator(color: _blue))
              : TabBarView(controller: _tab, children: [
            _tabBody(requests, emptyTitle: 'No pending requests',
                emptySub: 'New requests will appear here',
                item: (b) => _RequestCard(booking: b,
                    onAccept: () => _respond(b.id, 'accept'),
                    onReject: () => _respond(b.id, 'reject'))),
            _tabBody(confirmed, emptyTitle: 'No upcoming stays',
                emptySub: 'Confirmed bookings will appear here',
                item: (b) => _UpcomingCard(booking: b, onComplete: () => _complete(b.id))),
            _tabBody(history, emptyTitle: 'No history yet',
                emptySub: 'Completed bookings appear here',
                item: (b) => _HistoryCard(booking: b)),
          ])),
        ]),
      );
    });
  ...
  Tab _tabLabel(String label, int count, Color color) => Tab(child: Row(mainAxisSize: MainAxisSize.min, children: [
    Text(label),
    if (count > 0) ...[SizedBox(width: 5.w),
  ...
  Widget _tabBody(List<StayBookingModel> items, {
    required String emptyTitle, required String emptySub,
    required Widget Function(StayBookingModel) item,
  }) {
    if (items.isEmpty) return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      SizedBox(height: 6.h),
      Text(emptySub, style: TextStyle(color: _gray, fontSize: 13.sp), textAlign: TextAlign.center),
    ]));
    return RefreshIndicator(color: _blue, onRefresh: () async {
      ...
      itemBuilder: (_, i) => Padding(padding: EdgeInsets.only(bottom: 12.h), child: item(items[i])),
    ));
  }
}
...
class _RequestCard extends StatelessWidget {
      ...
      child: Padding(padding: const EdgeInsets.all(16), child: Column(
        ...
        Container(padding: const EdgeInsets.all(12),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _row(CupertinoIcons.house_fill, booking.stayName.isNotEmpty ? booking.stayName : 'Stay'),
              const SizedBox(height: 5),
              _row(CupertinoIcons.calendar, '${_fd(booking.checkinDate)} → ${_fd(booking.checkoutDate)}  (${booking.totalNights}N)'),
              const SizedBox(height: 5),
              ...
              if (booking.specialNote?.isNotEmpty == true) ...[const SizedBox(height: 5),
  ...
  Widget _row(IconData i, String t) => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Icon(i, size: 13, color: _gray), const SizedBox(width: 8),
...
class _UpcomingCard extends StatelessWidget {
  ...
  const _UpcomingCard({required this.booking, required this.onComplete});

  String _fd(String d) {
    try { return DateFormat('dd MMM').format(DateTime.parse(d)); } catch (_) { return d; }
  }
      ...
      child: Column(children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
            ...
            child: Row(children: [
              ...
              Text('${_fd(booking.checkinDate)} → ${_fd(booking.checkoutDate)}  · ${booking.totalNights}N',
        ...
        Padding(padding: const EdgeInsets.all(14), child: Row(children: [
          CircleAvatar(radius: 22, backgroundColor: Colors.grey.shade100,
              ...
              child: booking.touristPhoto.isEmpty
                  ? Text(booking.touristFullName.isNotEmpty ? booking.touristFullName[0].toUpperCase() : 'G',
                  style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14)) : null),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(booking.touristFullName.isNotEmpty ? booking.touristFullName : 'Guest',
                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: _dark), overflow: TextOverflow.ellipsis),
            ...
            if (booking.touristPhone.isNotEmpty)
              Row(children: [Icon(CupertinoIcons.phone, size: 11, color: _green), const SizedBox(width: 4),
        ...
        Padding(padding: const EdgeInsets.fromLTRB(14, 0, 14, 14), child: SizedBox(
          ...
          child: ElevatedButton.icon(onPressed: onComplete,
    ...
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
        CircleAvatar(radius: 20, backgroundColor: Colors.grey.shade100,
            ...
            child: booking.touristPhoto.isEmpty
                ? Text(booking.touristFullName.isNotEmpty ? booking.touristFullName[0].toUpperCase() : 'G',
                style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13)) : null),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(booking.touristFullName.isNotEmpty ? booking.touristFullName : 'Guest',
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5),
            decoration: BoxDecoration(color: _col.withOpacity(0.1), borderRadius: BorderRadius.circular(20),
                border: Border.all(color: _col.withOpacity(0.3))),
            child: Text(_lbl, style: TextStyle(color: _col, fontSize: 11, fontWeight: FontWeight.w700))),
      ])),
    );
  }
}