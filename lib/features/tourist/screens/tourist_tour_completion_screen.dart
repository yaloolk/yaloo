...
class _TouristTourCompletionScreenState
  ...
  int    _stars      = 0;
  int?   _tipIndex;          // index into _tipOptions
  String _customTip  = '';
  ...
  void didChangeDependencies() {
    ...
    if (args is GuideBookingModel && _booking == null) {
      _booking = args;
    }
  }
  ...
  Future<void> _submit() async {
    if (_stars == 0) {
      _snack('Please select a star rating', _amber);
      return;
    }
    ...
    try {
      await _api.post('/accounts/reviews/', data: {
        ...
        if (_tipIndex != null && _tipOptions[_tipIndex!] > 0)
          'tip_amount': _tipOptions[_tipIndex!],
        if (_tipIndex == null && _customTip.isNotEmpty)
          'tip_amount': int.tryParse(_customTip) ?? 0,
      });
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bg,
      body: SafeArea(
        child: _submitted ? _thankyouView() : _feedbackView(_booking),
      ),
    );
  }
    ...
    child: Column(children: [
      ...
      Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(FontAwesomeIcons.flagCheckered, color: Colors.white, size: 14.w),
          SizedBox(width: 8.w),
          Text('Tour Completed!',
              style: TextStyle(
                  color: Colors.white,
      ...
      Container(
        ...
        child: CircleAvatar(
            ...
            backgroundImage: b?.guidePhoto.isNotEmpty == true
                ? CachedNetworkImageProvider(b!.guidePhoto)
                : null,
            child: b?.guidePhoto.isNotEmpty != true
                ? Icon(CupertinoIcons.person, color: Colors.white, size: 36.w)
                : null),
      ),
      SizedBox(height: 16.h),

      Text(b?.guideName.isNotEmpty == true ? b!.guideName : 'Your Guide',
          style: TextStyle(
              color: Colors.white,
              fontSize: 20.sp,
              fontWeight: FontWeight.w800)),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextField(
        ...
        onChanged: (v) => setState(() {
          _customTip = v;
          if (v.isNotEmpty) _tipIndex = null;
        }),
      ),
    ]),
  );
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(
        ...
        child: OutlinedButton.icon(
          onPressed: () {
            if (b == null) return;
            Navigator.pushNamed(context, '/guideDetail',
